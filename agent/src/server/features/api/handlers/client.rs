...
use crate::server::{entity::Client, migration::get_connection, middleware::AuthUser};
...
pub async fn list_clients(Extension(auth_user_opt): Extension<Option<AuthUser>>) -> impl IntoResponse {
    ...
    let clients = if auth_user.is_admin {
        ...
        if user_client_ids.is_empty() {
            ...
            match Client::find()
                .filter(crate::server::entity::client::Column::Id.is_in(user_client_ids))
                .all(db)
                .await
            {
                Ok(clients) => clients,
...
pub async fn create_client(
    ...
    let new_client = crate::server::entity::client::ActiveModel {
        id: NotSet,
        name: Set(req.name),
    ...
    match new_client.insert(db).await {
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                ApiResponse::<crate::server::entity::client::Model>::error(format!(
...
pub async fn get_client(Path(id): Path<i64>, Extension(_auth_user): Extension<Option<AuthUser>>) -> impl IntoResponse {
    ...
    match Client::find_by_id(id).one(db).await {
        ...
        Ok(None) => (
            ...
            ApiResponse::<crate::server::entity::client::Model>::error("Client not found".to_string()),
        ),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            ApiResponse::<crate::server::entity::client::Model>::error(format!(
...
pub async fn update_client(
    ...
    let auth_user = match auth_user_opt {
        Some(user) => user,
        None => return (StatusCode::UNAUTHORIZED, ApiResponse::<crate::server::entity::client::Model>::error("Not authenticated".to_string())),
    };

    if !auth_user.is_admin {
        return (StatusCode::FORBIDDEN, ApiResponse::<crate::server::entity::client::Model>::error("Only admin can update client".to_string()));
    }
    ...
    let client = match Client::find_by_id(id).one(db).await {
        Ok(Some(c)) => c,
        Ok(None) => return (StatusCode::NOT_FOUND, ApiResponse::<crate::server::entity::client::Model>::error("Client not found".to_string())),
        Err(e) => return (StatusCode::INTERNAL_SERVER_ERROR, ApiResponse::<crate::server::entity::client::Model>::error(format!("Failed to find client: {}", e))),
    };

    let mut client_active: crate::server::entity::client::ActiveModel = client.into();
    ...
    match client_active.update(db).await {
        ...
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, ApiResponse::<crate::server::entity::client::Model>::error(format!("Failed to update client: {}", e))),
    }
}
