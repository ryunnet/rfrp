        ...
        >
          ...
          <CardContent className="space-y-4">
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              {/* 记住用户名 */}
              <div className="flex items-center gap-2 login-stagger-5">
                <input
                  type="checkbox"
                  id="remember"
                  checked={rememberUsername}
                  onChange={(e) => setRememberUsername(e.target.checked)}
                  className="w-3.5 h-3.5 rounded border-white/20 bg-white/[0.06] text-cyan-500 focus:ring-cyan-500/30 focus:ring-offset-0 cursor-pointer"
                  disabled={loading}
                />
                <label htmlFor="remember" className="text-xs text-white/35 cursor-pointer select-none">
                  记住用户名
                </label>
              </div>
            ...
            <div className="w-full border-t border-white/[0.06] pt-4 space-y-3">
              {registerEnabled && (
                <p className="text-center text-sm text-white/45">
                  没有账号？{' '}
                  <Link to="/register" className="text-white/75 hover:text-white font-medium transition-colors">
                    立即注册
                  </Link>
                </p>
              )}
